        ...
        <div class="box-header">
            <form class="form-inline" action="{{url('purchase/supplier')}}" method="get">
                ...
                <div class="pull-right">
                    <a href="javascript:;" class="btn btn-add btn-primary pull-right btn-create">添加用户</a>
                </div>
            </form>
        </div>
            <div class="row">
...
<script>
    $(document).ready(function () {
        ...
        $('#table_list_2').on("click", ".btn-user-role", function () {
            ...
            $(location).attr('href', '/admin/roles/userrolelist/' + id);
        });
        ...
        $('#table_list_2').on("click", ".btn-authorize", function () {
            ...
            $.ajax({
                type: "get",
                url: "{{url("linkage/role/authorize")}}",
                data: {role_id: id},
                async: false,
        ...
        $('#table_list_2').on("click", ".btn-edit", function () {
            var id = $(this).attr("data-id");

            $.ajax({
                type: "get",
                url: "{{url("permission/users")}}/" + id + "/edit",
                data: {id: id},
                async: false,
                success: function (ret) {
                    if (ret.code == 200) {
        ...
        $('#table_list_2').on("click", ".btn-del", function () {
            ...
            bootbox.confirm("是否删除？", function (result) {
                if (result) {
                    $.post('{{url("linkage/role/delete")}}', {'id': id}, function (ret) {
                        if (ret.code == 200) {
